...
public:
    ...
    virtual void LoadFile() const = 0; // Virtual Function to output file
...
public:
    ...
    void loadtofile()
    {
        ...
        ofstream O("Others.txt",ios::trunc);
        for(int i=0; i<itemC; i++)
        item[i]->LoadFile();
    }
...
public:
    ...
    void viewSuppliers()
    {
        for (int i = 0; i < supplierCount; i++)
        {
            cout << "Name: " << suppliers[i]->getName() << ", Contact: " << suppliers[i]->getContact() << endl;
        }
    }
    void loadtofile()
    {
